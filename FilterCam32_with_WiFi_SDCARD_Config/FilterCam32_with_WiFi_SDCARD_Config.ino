...
//Modified from:      https://arduinogetstarted.com/tutorials/arduino-read-config-from-sd-card
#define FILE_NAME "/config.txt"
// Create a text file called Config.txt 
//LINE 0: SSID=YOUR_SSID_NAME
//LINE 1: PASS=YOUR_WIFI_PASSWORD

#define KEY_MAX_LENGTH      80 // change it if key is longer
#define VALUE_MAX_LENGTH    80 // change it if value is longer

#define EEPROM_SIZE         64

#define SSID_LOCATION        0x0
#define PASS_LOCATION        0x25
...
//    WiFi SETUP   /////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void WifiSetup(const char* sSSID, const char* sPASS) {
    ...
    if (stationCount > firstStation) {
        for (int i=firstStation; i < stationCount; i++) Serial.printf(" '%s'", sSSID);
    } else {
        Serial.print("None");
    }
    Serial.println();
        ...
        if (stationsFound > 0) {
            for (int i = 0; i < stationsFound; ++i) {
                ...
                for (int sta = firstStation; sta < stationCount; sta++) {
                    if ((strcmp(sSSID, thisSSID.c_str()) == 0) ||
                    (strcmp(sSSID, thisBSSID.c_str()) == 0)) {
                        Serial.print("  -  Known!");
    ...
    if (bestStation == -1) {
        ...
        WiFi.begin(bestSSID, sPASS, 0, bestBSSID);
    ...
    if (accesspoint && (WiFi.status() != WL_CONNECTED)) {
        // The accesspoint has been enabled, and we have not connected to any existing networks
            ...
            WiFi.softAP(sSSID, sPASS, AP_CHAN);
        # else
...
void setup() {
  ...
  if(!SD_MMC.begin()){
    Serial.println("SD Card Mount Failed, :(");
        ...
        //Serial.println(" if(!SD_MMC.begin()){ }else{}");
        Serial.println("SD Card Mounted!, :)");
        csSSID = SD_findString(F("SSID"));
        csPASS = SD_findString(F("PASS"));

        if (csSSID.length() > 0 && csPASS.length() > 0 ){
          // We only want to write SSID and PASS Keys to EEPROM if they were found in Config.txt
          //Serial.println("if (csSSID.length() > 0 && csPASS.length() > 0 )");

          Serial.print("SSID from KEY: ");
          Serial.println(csSSID);
          Serial.print("PASS from KEY: ");
          Serial.println(csPASS);
          ...
          writeStringToEEPROM(SSID_LOCATION, sSSID);
          writeStringToEEPROM(PASS_LOCATION, sPASS);
        }else{
          Serial.println("SSID & PASS were not found in Config.txt, or the Config.txt is not on this inserted card.");
        }
    SD_MMC.end();
  }
...
/*
  ////////////////////////////////////////////////////////////////////////////////////
  Setting WifiSetup(sSSID, sPASS); sSSID for ssid, and sPASS for password from EEPROM
  ////////////////////////////////////////////////////////////////////////////////////
*/


   // We always read the SSID and PASS from the EEPROM even if it was just stored above.
  String charSSID = readStringFromEEPROM(SSID_LOCATION);
  String charPASS = readStringFromEEPROM(PASS_LOCATION);

  EEPROM.end();
// If the sum of two lengths of charSSID and charPASS are greater than zero we assume the data they have is valid.
  if ((charSSID.length() + charPASS.length())> 0){
      sSSID = charSSID.c_str();
      sPASS  = charPASS.c_str();

      Serial.print("SSID from EEPROM: "); Serial.println(charSSID);
      Serial.print("PASS from EEPROM: "); Serial.println(charPASS);
      
  }else{
      Serial.println("=============================================================================");
      Serial.println("=============================================================================");
      Serial.println("Password or SSID were not found in Config.txt on the SDCard or in the EEPROM.");
      Serial.println("=============================================================================");
      Serial.println("=============================================================================");
      Serial.println("Closing connections and restartarting in 10 Seconds.");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//    For reading KEY strings fron  Config.txt    //////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Modified from:      https://arduinogetstarted.com/tutorials/arduino-read-config-from-sd-card
...
int SD_findKey(const __FlashStringHelper * key, char * value) {
  File configFile = SD_MMC.open(FILE_NAME);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Modified from:  https://stackoverflow.com/questions/56139657/write-string-to-permanent-flash-memory-of-arduino-esp32
//And:            https://www.aranacorp.com/en/using-the-eeprom-with-the-esp32/

void writeStringToEEPROM(int addrOffset, const String &strToWrite){
    byte dataLength = strToWrite.length();

    EEPROM.write(addrOffset, dataLength);  //Store the length of string to specified address slot 
      // writing byte-by-byte to EEPROM
    for (int iAddress = 0; iAddress <  dataLength; iAddress++) {
        EEPROM.write((addrOffset  + 1 + iAddress), strToWrite[iAddress]);
        //addr += 1;
    }
    EEPROM.commit();
}
  ...
  {
    //Serial.println("");
    //Serial.print("readStringFromEEPROM(): ");
    //Serial.println("");
    ...
    byte dataLength = EEPROM.read(addrOffset);
    ...
    for (int iAddress = 0; iAddress < dataLength; iAddress++) {
        byte readValue = EEPROM.read(addrOffset + 1 + iAddress);
        ...
        readValueChar += String(char(readValue));
